...
function HistoryTab() {
  ...
  const fetchAll = useCallback(async (d: string) => {
    ...
    try {
      ...
      const ciData = await ciRes.json(); const apptData = await apptRes.json()
      setHistory(ciData.queue ?? []); setAppts(apptData.appointments ?? [])
    } catch { /* silent */ }
    finally { setLoading(false) }
  }, [])
  ...
  const waitMins = history.filter(e => e.served_at)
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        {date !== todayDate() && (
          <button onClick={() => setDate(todayDate())}
            className="text-xs tracking-widest uppercase text-gold/50 hover:text-gold font-sans transition-colors duration-200">Today</button>
        )}
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="glass-card border border-border rounded-sm px-5 py-4">
                ...
                <p className="font-serif text-3xl text-gold">
                  {appts.filter(a => a.checked_in).length}<span className="text-base text-gold/40">/{appts.length}</span>
                </p>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-2">
            <p className="text-[10px] tracking-[0.25em] uppercase text-offwhite/30 font-sans">Walk-In Check-Ins — {history.length}</p>
            {history.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 border-t border-border/30 pt-6">
            <div className="flex items-center gap-4">
              <p className="text-[10px] tracking-[0.25em] uppercase text-offwhite/30 font-sans">Appointment Records — {appts.length}</p>
              {appts.length > 0 && (
                <div className="flex gap-3 ml-auto">
                  <span className="text-[10px] font-sans text-gold/50">✓ {appts.filter(a => a.checked_in).length} showed</span>
                  {appts.filter(a => !a.checked_in).length > 0 && (
                    <span className="text-[10px] font-sans text-red-400/60">✕ {appts.filter(a => !a.checked_in).length} no-show</span>
                  )}
                </div>
              )}
            </div>
            {appts.length === 0 ? (
              <div className="glass-card border border-border/40 rounded-sm p-8 text-center">
                <p className="text-offwhite/20 font-sans text-sm">No appointments scheduled.</p>
              </div>
            ) : (
              <>
                ...
                {appts.map((appt, idx) => {
                  const isPast = date < todayDate(); const isNoShow = isPast && !appt.checked_in
                  return (
                    <motion.div key={appt.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      className={`glass-card border rounded-sm px-5 py-3 grid grid-cols-[64px_1fr_1fr_80px] gap-4 items-center ${
                        isNoShow ? 'border-red-400/20 bg-red-400/5' : appt.checked_in ? 'border-border/30 opacity-50' : 'border-border/50'
                      }`}>
                      <p className="text-xs font-sans text-gold/50">{fmt12h(appt.appointment_time)}</p>
                      <div className="min-w-0">
                        <p className={`font-serif text-sm truncate ${appt.checked_in ? 'text-offwhite/40 line-through' : 'text-offwhite'}`}>{appt.customer_name}</p>
                      ...
                      <p className={`text-[10px] tracking-widest uppercase font-sans font-medium ${
                        isNoShow ? 'text-red-400/70' : appt.checked_in ? 'text-gold/60' : 'text-offwhite/25'
                      }`}>
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        ...
        <h1 className="font-serif text-3xl text-offwhite">Walk-In Queue</h1>
      </div>
        ...
        {(['queue', 'history'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-5 py-2.5 text-xs tracking-widest uppercase font-sans transition-all duration-200 border-b-2 -mb-px ${
              tab === t ? 'border-gold text-gold' : 'border-transparent text-offwhite/35 hover:text-offwhite/60'
            }`}>
