...
export default function AdminCustomersPage() {
  ...
  useEffect(() => { if (admin) fetchCustomers(admin.id) }, [admin, fetchCustomers])
  ...
  const openCustomer = useCallback(async (email: string) => {
    ...
    try {
      const [histRes, notesRes] = await Promise.all([
        fetch(`/api/phonebook?adminId=${admin.id}&email=${encodeURIComponent(email)}`),
        fetch(`/api/customer-notes?adminId=${admin.id}&email=${encodeURIComponent(email)}`),
      ])
  ...
  const handleAddNote = async () => {
    if (!admin || !expandedEmail || !noteText.trim()) return
    setNoteLoading(true)
    try {
      const res = await fetch('/api/customer-notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customerEmail: expandedEmail, note: noteText.trim(), adminId: admin.id }),
      })
      const data = await res.json()
      if (res.ok) {
  ...
  const handleDeleteNote = async (noteId: string) => {
    if (!admin) return
    await fetch(`/api/customer-notes?id=${noteId}&adminId=${admin.id}`, { method: 'DELETE' })
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center gap-3">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search by name, email, or phone…" className={ic} />
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {filtered.map((c, idx) => (
            <motion.div key={c.email} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.15, delay: idx * 0.02 }} className="flex flex-col">
              <button onClick={() => openCustomer(c.email)}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
                    ...
                    {c.phone && <span className="text-xs font-sans text-offwhite/50">{c.phone}</span>}
                  </div>
                          ...
                          <div className="flex flex-col gap-3">
                            ...
                            <div className="flex gap-2">
                              <input type="text" value={noteText} onChange={e => setNoteText(e.target.value)}
                                ...
                                placeholder="Add a note…" className={`${ic} text-xs py-1.5`} />
                                ...
                                {detail.notes.map(n => (
                                  <div key={n.id} className="flex items-start justify-between gap-3 border border-border/40 rounded-sm px-3 py-2">
                                    <div className="min-w-0">
                                      ...
                                      <p className="text-[10px] font-sans text-offwhite/25 mt-0.5">{n.created_by} · {fmtDateTime(n.created_at)}</p>
                                    </div>
                                    <button onClick={() => handleDeleteNote(n.id)}
                                      className="text-offwhite/20 hover:text-red-400 text-xs font-sans transition-colors duration-150 shrink-0">✕</button>
                                  </div>
                                ))}
                              </div>
                            )}
                              ...
                              <div className="flex flex-col gap-1.5">
                                {detail.history.map(h => (
                                  <div key={h.id} className="flex items-center gap-3 py-2 border-b border-border/20 last:border-0">
                                    ...
                                    {h.checked_in && <span className="text-[9px] tracking-widest uppercase text-gold/40 font-sans shrink-0">✓</span>}
                                  </div>
                                ))}
                              </div>
                            )}
