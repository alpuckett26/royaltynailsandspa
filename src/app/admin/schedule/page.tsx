...
function fmtDisplay(date: Date) { return date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) }
function fmtFull(date: Date) { return date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) }
        ...
        {DAYS.map((day, i) => {
          ...
          return (
            <div key={day} className="flex flex-col gap-2 min-h-[120px]">
              ...
              >
                <p className={`text-[10px] tracking-widest uppercase font-sans ${isToday ? 'text-gold' : 'text-offwhite/30 group-hover:text-offwhite/60'}`}>{day}</p>
                <p className={`text-sm font-serif ${isToday ? 'text-gold' : 'text-offwhite/50 group-hover:text-offwhite/70'}`}>{date ? date.getDate() : ''}</p>
        ...
        {modalDate && (
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                {modalRows.map((row, idx) => (
                  <div key={idx} className="grid grid-cols-[1fr_80px_80px_20px] gap-2 items-center">
                    ...
                    >
                      <option value="" className="bg-charcoal text-offwhite">Employee…</option>
                      {employees.map(emp => <option key={emp.id} value={emp.id} className="bg-charcoal text-offwhite">{emp.name}</option>)}
                    </select>
              ...
              {modalError && <p className="text-xs text-red-400 font-sans">{modalError}</p>}
