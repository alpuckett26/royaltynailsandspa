...
export default function AdminSchedulePage() {
  ...
  useEffect(() => {
    if (!admin) return
    fetch('/api/employee/list').then(r => r.json()).then(d => setEmployees(d.employees ?? []))
  }, [admin])
  ...
  const fetchShifts = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const from = fmt(weekDates[0]); const to = fmt(weekDates[6])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/employee/shifts', {
        ...
        body: JSON.stringify({
          adminId: admin.id, employeeId: formEmp, shiftDate: formDate,
          startTime: formStart, endTime: formEnd, notes: formNotes.trim() || null,
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error)
      setFormMsg({ type: 'ok', text: 'Shift added.' })
      setFormNotes(''); fetchShifts()
    } catch (err: unknown) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => setWeekOffset(w => w - 1)} className="text-offwhite/40 hover:text-offwhite transition-colors duration-200 text-lg">←</button>
          ...
          return (
            <div key={day} className="flex flex-col gap-2 min-h-[120px]">
              <div className={`text-center pb-2 border-b ${isToday ? 'border-gold/40' : 'border-border/40'}`}>
                ...
                <p className={`text-sm font-serif ${isToday ? 'text-gold' : 'text-offwhite/50'}`}>{date ? date.getDate() : ''}</p>
              </div>
              {loading ? (
                ...
                <div className="flex flex-col gap-1">
                  {dayShifts.map(shift => (
                    <motion.div key={shift.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                      ...
                      {shift.notes && <p className="text-[9px] font-sans text-offwhite/25 leading-tight truncate">{shift.notes}</p>}
                      <button onClick={() => handleDelete(shift.id)}
                        className="absolute top-0.5 right-0.5 opacity-0 group-hover:opacity-100 text-offwhite/20 hover:text-red-400 text-[10px] transition-all duration-150 leading-none">✕</button>
                    </motion.div>
                  ))}
                </div>
              )}
        ...
        {showForm && (
          <motion.form initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={formEmp} onChange={e => setFormEmp(e.target.value)} required className={ic}>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="date" value={formDate} onChange={e => setFormDate(e.target.value)} required className={ic} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] tracking-widest uppercase text-offwhite/35 font-sans">Start</label>
              <input type="time" value={formStart} onChange={e => setFormStart(e.target.value)} required className={ic} />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] tracking-widest uppercase text-offwhite/35 font-sans">End</label>
              <input type="time" value={formEnd} onChange={e => setFormEnd(e.target.value)} required className={ic} />
            </div>
            <div className="flex flex-col gap-1.5 col-span-2 sm:col-span-1">
              <label className="text-[10px] tracking-widest uppercase text-offwhite/35 font-sans">Notes (optional)</label>
              <input type="text" value={formNotes} onChange={e => setFormNotes(e.target.value)} placeholder="e.g. Open shift" className={ic} />
            </div>
